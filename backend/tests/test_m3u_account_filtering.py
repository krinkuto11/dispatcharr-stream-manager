...
class TestM3UAccountFiltering(unittest.TestCase):
    ...
    @patch('automated_stream_manager.refresh_m3u_playlists')
    @patch('automated_stream_manager.get_streams')
    def test_refresh_excludes_custom_streams(self, mock_asm_get_streams, mock_refresh, mock_api_get_streams):
        """Test that custom streams are excluded when fetching streams during refresh."""
        # Mock both the imported get_streams and the api_utils version
        mock_asm_get_streams.return_value = []
        mock_api_get_streams.return_value = []
        ...
        with patch('automated_stream_manager.CONFIG_DIR', Path(self.temp_dir)):
            ...
            # Check the dynamically imported get_streams in refresh_playlists
            self.assertEqual(mock_api_get_streams.call_count, 2)
            for call_obj in mock_api_get_streams.call_args_list:
